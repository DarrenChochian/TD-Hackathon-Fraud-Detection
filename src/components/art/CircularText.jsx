...
const CircularText = ({ text, spinDuration = 20, onHover = 'speedUp', size = 'sm', className = '' }) => {
  const letters = Array.from(text);
  const controls = useAnimation();
  const rotation = useMotionValue(0);
  const { container: containerSize, text: textSize } = SIZES[size] || SIZES.sm;
    ...
    >
      {letters.map((letter, i) => {
        ...
        return (
          <span
            key={i}
            className={`absolute inline-block inset-0 transition-all duration-500 ease-[cubic-bezier(0,0,0,1)] ${textSize}`}

...
> Add a user message to an existing thread with optional file attachments. Can send text only, attachments only, or both. Documents must be indexed before further messages are allowed. To choose a model, set llm_provider and model_name. If omitted, defaults are llm_provider=openai and model_name=gpt-4o. Set memory='Auto' to enable memory search and automatic memory operations. Supported attachment types: .pdf, .doc(x), .ppt(x), .xls(x), .txt, .csv, .md, .json(l), .xml, .py, .js, .ts, .jsx, .tsx, .html, .css, .cpp, .c, .h, .java, .go, .rs, .rb, .php, .sql, .png, .jpg, .jpeg, .webp, .gif, .bmp, .tiff, .tif. See the Model Library page in the dashboard (model-library page) for a current list of supported models and providers.

## OpenAPI

```yaml openapi-filtered.json post /threads/{thread_id}/messages
openapi: 3.1.0
info:
  title: Backboard API
  description: >-
...
paths:
  /threads/{thread_id}/messages:
    post:
      tags:
        - Threads
      summary: Add Message to Thread with Optional Attachments
      description: >-
        Add a user message to an existing thread with optional file attachments. Can send text only, attachments only,
        or both. Documents must be indexed before further messages are allowed. To choose a model, set llm_provider and
        model_name. If omitted, defaults are llm_provider=openai and model_name=gpt-4o. Set memory='Auto' to enable
        memory search and automatic memory operations. Supported attachment types: .pdf, .doc(x), .ppt(x), .xls(x),
        .txt, .csv, .md, .json(l), .xml, .py, .js, .ts, .jsx, .tsx, .html, .css, .cpp, .c, .h, .java, .go, .rs, .rb,
        .php, .sql, .png, .jpg, .jpeg, .webp, .gif, .bmp, .tiff, .tif. See the Model Library page in the dashboard
        (model-library page) for a current list of supported models and providers.
      operationId: add_message_to_thread_threads__thread_id__messages_post
      security:
        - APIKeyHeader: []
      parameters:
        - name: thread_id
      ...
      requestBody:
        content:
          multipart/form-data:
            schema:
              $ref: '#/components/schemas/Body_add_message_to_thread_threads__thread_id__messages_post'
      responses:
        '200':
...
components:
  schemas:
    Body_add_message_to_thread_threads__thread_id__messages_post:
      properties:
        content:
          anyOf:
            - type: string
        ...
        llm_provider:
          anyOf:
            ...
            - type: 'null'
          title: Llm Provider
          description: 'LLM provider name (nullable). Default: openai.'
        model_name:
          anyOf:
            - type: string
            - type: 'null'
          title: Model Name
          description: 'Model name (nullable). Default: gpt-4o.'
        stream:
          type: boolean
          title: Stream
          default: false
        memory:
          anyOf:
            ...
            - type: 'null'
          title: Memory
          description: 'Memory mode: ''Auto'' (search and write), ''off'' (disabled), ''Readonly'' (search only). Default: off.'
          default: 'off'
        web_search:
          anyOf:
            - type: string
            - type: 'null'
          title: Web Search
          description: 'Web search mode: ''Auto'' (enables web search tool for LLM), ''off'' (disabled). Default: off.'
          default: 'off'
        send_to_llm:
          anyOf:
            - type: string
          ...
          title: Send To Llm
          description: >-
            Whether to send the message to the LLM for a response. If 'false', the message is saved but no AI response
        ...
        metadata:
          anyOf:
            ...
            - type: 'null'
          title: Metadata
          description: Optional metadata as JSON string. Can include custom_timestamp (ISO 8601 format) and other custom fields.
        files:
          items:
            type: string
            format: binary
          ...
          default: []
      type: object
      title: Body_add_message_to_thread_threads__thread_id__messages_post
    MessageResponse:
      properties:
        message:
          type: string
      ...
      description: Response for message operations - includes content at top level for easy access
    MessageRole:
      type: string
      enum:
        - user

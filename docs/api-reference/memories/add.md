...
> Add a new memory directly for a specific assistant (manual creation from UI).

## OpenAPI

```yaml openapi-filtered.json post /assistants/{assistant_id}/memories
openapi: 3.1.0
info:
  title: Backboard API
  description: >-
  ...
  /assistants/{assistant_id}/memories:
    post:
      tags:
        - Memories
      summary: Add Memory
      description: Add a new memory directly for a specific assistant (manual creation from UI).
      operationId: add_memory_assistants__assistant_id__memories_post
      security:
        - APIKeyHeader: []
      parameters:
        - name: assistant_id
          ...
          content:
            application/json:
              schema:
                ...
                title: Response Add Memory Assistants  Assistant Id  Memories Post
        '422':

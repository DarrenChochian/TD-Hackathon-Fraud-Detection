...
> Delete a specific memory from both database and vector store.

## OpenAPI

```yaml openapi-filtered.json delete /assistants/{assistant_id}/memories/{memory_id}
openapi: 3.1.0
info:
  title: Backboard API
  description: >-
...
paths:
  /assistants/{assistant_id}/memories/{memory_id}:
    delete:
      ...
      description: Delete a specific memory from both database and vector store.
      operationId: delete_memory_assistants__assistant_id__memories__memory_id__delete
      security:
        - APIKeyHeader: []
      parameters:
        - name: assistant_id

...
```yaml openapi-filtered.json post /assistants/{assistant_id}/documents
openapi: 3.1.0
info:
  title: Backboard API
  description: >-
...
paths:
  /assistants/{assistant_id}/documents:
    post:
      tags:
        - Assistants
      summary: Upload Document to Assistant
      description: >-
        Upload a document to be associated with a specific assistant (shared context for its threads). Supported file
        types: **Documents** (.pdf, .doc, .docx, .ppt, .pptx, .xls, .xlsx), **Text/Data** (.txt, .csv, .md, .markdown,
        .json, .jsonl, .xml), **Code** (.py, .js, .ts, .jsx, .tsx, .html, .css, .cpp, .c, .h, .java, .go, .rs, .rb,
        .php, .sql), **Images** (.png, .jpg, .jpeg, .webp, .gif, .bmp, .tiff, .tif).
      operationId: upload_document_to_assistant_assistants__assistant_id__documents_post
      security:
        - APIKeyHeader: []
      parameters:
        - name: assistant_id
      ...
      requestBody:
        ...
        content:
          multipart/form-data:
            schema:
              $ref: '#/components/schemas/Body_upload_document_to_assistant_assistants__assistant_id__documents_post'
      responses:
        '200':
...
components:
  schemas:
    Body_upload_document_to_assistant_assistants__assistant_id__documents_post:
      properties:
        file:
          type: string
          format: binary
      ...
      required:
        - file
      title: Body_upload_document_to_assistant_assistants__assistant_id__documents_post
    DocumentRead:
      properties:
        metadata_:
          anyOf:
